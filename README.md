...
EP5–20の `learningIntro`（`title`・`body`・`focus`）は、道具と読み方の導入です。各エピソードの最初のミッションで展開し、以後は折りたたんで参照できます。

各エピソードの最終問題は `quiz.kind: "decision"` とし、証拠に基づく判断・追加照合・報告を問います。`quiz.sources` に同じエピソードの先行ミッションIDを指定すると、クイズで出力を見返せます。選択肢は70文字以内で、時刻・対象・承認範囲などの読み違いを扱い、正答の位置だけで推測できないようにします。既存の20エピソード・96ミッションと進捗IDは維持しています。
...
スポンサー枠はレッスン完了後だけ表示し、「広告」と明示しています。A8.net経由の楽天市場商品リンク（Linux・ハッキング関連の参考書）を掲載しています。価格・在庫はリンク先を優先します。広告・プライバシー通知は [privacy.html](privacy.html) に公開しています。

現時点ではプログラマティック広告ネットワークを使っていないため、`ads.txt` は配置していません。別の表示広告を導入する場合は、広告事業者の指定に従って `ads.txt` と同意管理を追加します。

## Cloudflare Pages公開

Cloudflare PagesではFramework presetを使わず、Build commandを `npm test && npm run build`、Build output directoryを `dist` に設定します。`dist/404.html`、`dist/_headers`、`dist/robots.txt`、`dist/sitemap.xml` もビルドに含まれます。公開ドメイン（`https://trace-type.com/`）をcanonicalとOGPの基準URLにし、トップ、ラボ、説明ページ、安全方針、広告・プライバシー通知にメタデータを設定しています。`npm run build` は `data/missions.json` から、検索向けのエピソード概要ページ（`/lab/ep01/`〜`/lab/ep20/`）も `dist/` に生成します。概要ページから既存の `lab.html?episode=...` を開くため、ラボの進行ロジックと既存共有URLは維持します。
