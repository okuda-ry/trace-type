...
IT初学者とセキュリティ職志望者向けの、解説付きサイバーセキュリティ・タイピングラボです。TYPE → UNDERSTAND → 模擬出力 → クイズの順で、安全なコマンドの考え方を学びます。

## 安全方針

初期版は静的サイトです。アカウント、API、外部ネットワーク、実コマンド実行はありません。`example.test` と予約済みドキュメント用IP、JSON内の模擬ログだけを使います。入力はブラウザ内で評価し、進捗とミス数はlocalStorageに保存します。
