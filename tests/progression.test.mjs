...
test("privacy title keeps its phrase together on the public notice page", () => {
  ...
  assert.match(privacy, /<h1 id="privacyTitle" class="privacy-title">広告・プライバシー通知<\/h1>/);
  assert.match(homeCss, /\.home-hero h1\.privacy-title[\s\S]*?white-space: nowrap/);
});
...
test("index is the public home and lab remains the lesson page", () => {
  ...
  const lab = fs.readFileSync(new URL("../lab.html", import.meta.url), "utf8");
  const homeCss = fs.readFileSync(new URL("../home.css", import.meta.url), "utf8");
  assert.match(index, /<link rel="stylesheet" href="home\.css\?v=20260827-credit2">/);
  ...
  assert.match(index, /<footer class="home-footer">[\s\S]*<span class="home-footer-copy">TRACE \/ TYPE · タイピングで学ぶセキュリティ教材<\/span>[\s\S]*<span class="home-credit">created by okokp<\/span>/);
  assert.doesNotMatch(index, /ブラウザ内の学習環境/);
  ...
  assert.match(homeCss, /@media \(min-width: 40rem\)[\s\S]*?\.home-credit\s*\{[\s\S]*text-align:\s*right/);
