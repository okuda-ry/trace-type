...
test('outputs are simulated and quizzes valid',()=>{for(const m of flattenMissions(data)){assert.match(m.output,/SAFE SIMULATION/);assert.ok(m.quiz.answer>=0&&m.quiz.answer<m.quiz.choices.length);}});
...
test('expected correct choices match each mission',()=>{const expected={
  'ep01-m1':'現在の作業ディレクトリ','ep01-m2':'.case-idなど隠し項目も表示','ep01-m3':'模擬端末の現在の作業ディレクトリ','ep01-m4':'ops-traineeとNS-248',
